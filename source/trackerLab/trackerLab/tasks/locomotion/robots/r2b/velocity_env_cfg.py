...
class RobotSceneCfg(InteractiveSceneCfg):
    ...
    # robots
    robot: ArticulationCfg = R2_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
class EventCfg:
    ...
    push_robot = EventTerm(
        ...
        params={"velocity_range": {"x": (-0.8, 0.8), "y": (-0.8, 0.8), "z": (-0.2, 0.4)}},
    )
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        limit_ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-1.0, 1.5), lin_vel_y=(-0.8, 0.8), ang_vel_z=(-0.3, 0.3)
        ),
    )
...
class ActionsCfg:
    ...
    JointPositionAction = mdp.JointPositionActionCfg(
        asset_name="robot", joint_names=[".*"], scale=0.50, use_default_offset=True
    )
...
class RewardsCfg:
    ...
    is_terminal = RewTerm(func=mdp.is_terminated, weight=-100.0)
