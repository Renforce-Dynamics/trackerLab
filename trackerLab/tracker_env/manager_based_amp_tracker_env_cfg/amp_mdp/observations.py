...
def body_pos_w(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    ref_body_name = env.cfg.reference_body
    ref_body_index = asset.data.body_names.index(ref_body_name)
...
def key_body_pos_w(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """Root linear velocity in the asset's root frame."""
    # extract the used quantities (to enable type-hinting)
    key_body_names = env.cfg.key_body_names
    asset: RigidObject = env.scene[asset_cfg.name]
    ...
    ref_body_name = env.cfg.reference_body
    ref_body_index = asset.data.body_names.index(ref_body_name)
    root_positions = asset.data.body_pos_w[:, ref_body_index].unsqueeze(-2)
